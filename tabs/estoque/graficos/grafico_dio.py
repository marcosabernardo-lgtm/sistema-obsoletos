...
def render(df_hist, df_obsoleto, moeda_br, data_selecionada):
    ...
    for id_unico in pivot.index:
        ...
        cpv_list.append({"ID_UNICO": id_unico, "CPV_Calculado": sum(reducoes)})
    ...
    c2.markdown(f'<div class="card-dio"><div class="titulo">DIO Mediano</div><div class="valor">{f"{dio_mediano:.0f} dias" if dio_mediano is not None else "—"}</div><div class="sub" style="color:#ccc">{label_inicial} → {label_atual} ({n_meses}m)</div></div>', unsafe_allow_html=True)
    ...
    c4.markdown(f'<div class="card-dio"><div class="titulo">Giro Alto + Medio</div><div class="valor" style="color:#51cf66">{qtd_alto + qtd_medio} produtos</div><div class="sub" style="color:#51cf66">90 dias</div></div>', unsafe_allow_html=True)

    st.markdown("<br>", unsafe_allow_html=True)

    ordem = ["Giro Alto (<=30d)", "Giro Medio (31-90d)", "Giro Baixo (91-180d)", "Critico (>180d)", "Sem Consumo"]

    resumo = df_dio.groupby("Classificacao").agg(
    ...
    css_resumo = "<style>.tb-resumo{width:100%;border-collapse:collapse;font-size:13px;color:white;margin-bottom:24px;}.tb-resumo th{background-color:#0f5a60;color:white;padding:9px 12px;text-align:left;border-bottom:2px solid #EC6E21;font-weight:700;}.tb-resumo th:not(:first-child){text-align:right;}.tb-resumo td{padding:7px 12px;border-bottom:1px solid #1a6e75;background-color:#005562;}.tb-resumo td:not(:first-child){text-align:right;}</style>"
    st.html(css_resumo + "<table class='tb-resumo'><thead><tr><th>Classificacao</th><th>Produtos</th><th>Valor</th><th>% Estoque</th></tr></thead><tbody>" + linhas_resumo + "</tbody></table>")
    ...
    df_export = df_tabela.rename(columns={"Produto": "Codigo", "Descricao": "Descricao", "Custo Total Atual": "Estoque Final", "Estoque Medio": "Estoque Medio", "CPV 12m": "CPV (12m)", "DIO": "DIO (dias)"})
    colunas_excel = ["Codigo", "Descricao", "Conta", "Empresa / Filial", "Classificacao", "Saldo Inicial", "Estoque Final", "Estoque Medio", "CPV (12m)", "DIO (dias)", "Ult_Movimentacao"]
    ...
    st.download_button(label="Exportar para Excel", data=output.getvalue(), file_name="relatorio_dio.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
    ...
    css = "<style>.tb-dio{width:100%;border-collapse:collapse;font-size:13px;color:white;}.tb-dio th{background-color:#0f5a60;color:white;padding:9px 12px;text-align:left;border-bottom:2px solid #EC6E21;font-weight:700;white-space:nowrap;}.tb-dio th:nth-child(n+6){text-align:right;}.tb-dio td{padding:7px 12px;border-bottom:1px solid #1a6e75;background-color:#005562;color:white;}.tb-dio td:nth-child(n+6){text-align:right;}.tb-dio tr:hover td{background-color:#0a6570;}</style>"
    st.html(css + f"<p style='color:#aaa;font-size:12px'>{len(df_tabela)} produtos</p><table class='tb-dio'><thead><tr><th>Codigo</th><th>Descricao</th><th>Conta</th><th>Empresa / Filial</th><th>Classificacao</th><th>Saldo Inicial</th><th>Estoque Final</th><th>Estoque Medio</th><th>CPV (12m)</th><th>DIO (dias)</th><th>Ult Mov</th></tr></thead><tbody>" + linhas + "</tbody></table>")
